...
# 获取订单信息
def look_order_sql(date_content, username):
    conn = pymysql.connect(host='139.155.33.105', port=2707, user='root', password='Liyitong97!',  db='goodsorder',
                           charset='utf8')
    cursor = conn.cursor()
    sql1 = "SELECT * FROM `order` WHERE goodid LIKE '%{}%' OR date LIKE '%{}%';".format(date_content, date_content)
    print(sql1)
    ...
    #  判断是否有权限查看此订单
    if all_list[0]['buyer'] != username and all_list[0]['seller'] != username:
        all_list = []
