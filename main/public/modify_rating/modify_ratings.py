...
import json
from main.public.modify_rating.modify_ratings_sql import modify_ratings_sql
...
def modify_ratings_fun():
    ...
    # token对比成功
        ...
        modify_ratings_sql(get_data['text'], get_data['start'], get_data['to_examine'], get_data['good_id'], get_data['no'])
        post_data = {'info': '修改成功'}
        #  返回的内容
        response = make_response(json.dumps(post_data))
        #  返回的json格式设定
