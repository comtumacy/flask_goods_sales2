...
# 查看评论
def look_rating_sql(username, page_number):
    conn = pymysql.connect(host='139.155.33.105', port=2707, user='root', password='Liyitong97!', db='goods',
                           charset='utf8')
    cursor = conn.cursor()
    sql1 = "select * FROM ratings WHERE ratinguser = '{}' order by `Rid` LIMIT {}, 5;".format(username, (int(page_number) - 1) * 5)
    sql2 = "SHOW full COLUMNS FROM ratings"
    sql3 = "select COUNT(*) FROM ratings WHERE ratinguser = '{}';".format(username)
    cursor.execute(sql1)
    conn.commit()
