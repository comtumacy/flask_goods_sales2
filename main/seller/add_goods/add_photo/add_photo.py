...
def add_photo_fun():
    ...
    # token对比成功
        ...
        file = open('/home/flask/goods_sale/static/goods_photo/{}/good.jpg'.format(get_data['good_id']), 'wb')
        file.write(img)
        file.close()
        # 图片地址
        ...
        add_photo_sql(table_name, photo_url, get_data['good_id'])
        post_data = {'info': '图片上传保存成功'}
        #  返回的内容
        response = make_response(json.dumps(post_data))
        #  返回的json格式设定
        response.content_type = 'application/json'
