...
def add_good_fun():
    ...
    # token对比成功
        ...
        # 类型为书籍
        if get_data['type'] == 1:
            status = add_book_sql(get_data['good_id'], get_data['name'], get_data['price'], get_data['discount'], get_data['old_price'], get_data['author'], get_data['area'], get_data['publishing_time'], get_data['open_book'], get_data['paper'], get_data['packing'], get_data['suit'], get_data['introduction'], get_data['book_img'], get_data['stid'], get_data['new_and_old_degree'])
            if status == 1:
        ...
        # 类型为手机
            ...
            num = add_phone_sql(get_data['title'], get_data['price'], get_data['good_title'], get_data['good_weight'], get_data['good_from'], get_data['good_system'], get_data['signal'], get_data['screen_size'], get_data['img'], get_data['stid'])
            status = add_info_to_management_sql(username, int(num) + 1, get_data['stid'], time.strftime("%Y-%m-%d", time.localtime()))
            if status == 1:
                result = {'info': '电子设备添加成功'}
                # 设置返回对象
                response = make_response(json.dumps(result))
                # 设置状态码
                response.status_code = 200
